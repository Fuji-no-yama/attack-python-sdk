...
if __name__ == "__main__":
    attack = Attack(domain="enterprise", version="18.1", initialize_vector=True)
    print(f"テクニック数: {len(attack.technique_list)}")
    print(f"タクティック数: {len(attack.tactic_list)}")
    print(f"緩和策数: {len(attack.mitigation_list)}")
    print(f"キャンペーン数: {len(attack.campaign_list)}")
    print(f"ソフトウェア数: {len(attack.software_list)}")
    print(f"グループ数: {len(attack.group_list)}")
    ...
    print(f"最初のテクニックに関連する手順数: {len(first_technique.procedure_list)}")
